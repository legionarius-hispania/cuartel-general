...
/* Patrón compartido por todos los formularios y diálogos de confirmación
   que solo cambian el estado compartido (crear/editar/borrar un pedido,
   producto, material...): commit() ya pinta un primer render() mientras el
   formulario o el borrador de ui seguía con sus valores anteriores, así que
   sin un render() extra DESPUÉS de resetearlos (resetFn) se quedarían
   visibles hasta el siguiente cambio de pantalla en modo local/sin conexión
   (en el modo normal, con guardado en Supabase, el redibujado posterior al
   guardado lo disimulaba, por eso no se detectó hasta las pruebas de
   extremo a extremo de la auditoría del 2026-09-09). */
function commitAndReset(mutator, resetFn){
  commit(mutator);
  resetFn();
  render();
}
...
function renderOrders(){
  ...
  var rowsArr = filtered.map(function(o){
    ...
    var main = '<tr data-order-row="'+o.id+'">' +
      ...
      '<td>' + confirmDeleteActionsHtml('order', o.id) +
      '</td></tr>';
    var detail = '';
    if (expanded) {
...
function renderInventory(){
  ...
  var rowsArr = state.products.slice().sort(function(a,b){ return (a.name||'').localeCompare(b.name||''); }).map(function(p){
    ...
    var actions = (ui.confirm && ui.confirm.key === 'product:'+p.id) ?
      confirmDeleteActionsHtml('product', p.id) :
...
/* Celda de coste unitario de un DTF, igual en Material (renderMaterial /
   materialRowHtml) y en Costes (renderCosts → "Coste unitario por diseño
   DTF"): mismo dato, mismo formato, así que se calcula en un solo sitio en
   vez de repetir la misma expresión en las dos pantallas. */
function dtfUnitCostCell(m){
  return '<td class="stock-num">'+(num(m.altoCm,0) ? fmtMoney(dtfUnitCost(m)) : '<span class="muted">—</span>')+'</td>';
}
...
function materialRowHtml(m, cols){
  ...
  var actions = (ui.confirm && ui.confirm.key === 'material:'+m.id) ?
    confirmDeleteActionsHtml('material', m.id) :
...
/* Botón "Eliminar" (o, tras pulsarlo, "¿Seguro?"/"Cancelar") para
   cualquier tipo de fila borrable de la app — pedido, producto, material,
   imagen de referencia, máquina o entrada del libro de ingresos/gastos.
   `kind` identifica el tipo (coincide con el prefijo de ui.confirm.key) y
   se manda también en el propio botón para que el único par de acciones
   'ask-delete'/'confirm-delete' del onClick sepa a qué deleteX() llamar. */
function confirmDeleteActionsHtml(kind, id){
  if (ui.confirm && ui.confirm.key === kind+':'+id) {
    return '<button class="btn danger small" data-action="confirm-delete" data-kind="'+kind+'" data-id="'+id+'">¿Seguro?</button> ' +
      '<button class="btn ghost small" data-action="cancel-confirm">Cancelar</button>';
  }
  return '<button class="btn ghost small" data-action="ask-delete" data-kind="'+kind+'" data-id="'+id+'">Eliminar</button>';
}
...
function renderDtfImageLibrary(){
  ...
  var listItems = state.dtfImages.map(function(img){
    ...
    return '<div class="section" style="background:var(--surface-2);display:flex;gap:14px;align-items:flex-start;flex-wrap:wrap;">' +
      ...
      '</div>' +
      confirmDeleteActionsHtml('refimg', img.id) +
      '</div>';
  }).join('');
...
function renderCosts(){
  ...
  var machineRows = limitedRows('costMachines', machines.map(function(x){
    return '<tr><td>'+fmtDateShort(x.date)+'</td><td>'+esc(x.name)+'</td><td class="stock-num">'+fmtMoney(x.amount)+'</td>' +
      '<td>'+(x.note?esc(x.note):'<span class="muted">—</span>')+'</td><td>'+confirmDeleteActionsHtml('machine', x.id)+'</td></tr>';
  }), 5);
  ...
  var ledgerRows = limitedRows('costLedger', ledger.map(function(x){
    ...
    return '<tr><td>'+fmtDateShort(x.date)+'</td><td>'+badge+'</td><td>'+esc(x.concept)+'</td>' +
      ...
      '<td class="stock-num">'+fmtMoney(x.amount)+'</td><td>'+confirmDeleteActionsHtml('ledger', x.id)+'</td></tr>';
  }), 5);
...
function onClick(e){
  ...
  if (action === 'undo-fulfilled') { undoFulfilled(id); return; }
  ...
  /* Borrar cualquier tipo de fila (pedido, producto, material, imagen de
     referencia, máquina o entrada del libro): un único par de acciones,
     con el tipo en data-kind, en vez de un par "ask/confirm-delete-X" por
     cada pantalla (ver confirmDeleteActionsHtml). */
  if (action === 'ask-delete') { ui.confirm = {key: t.dataset.kind+':'+id}; render(); return; }
  if (action === 'confirm-delete') {
    var delKind = t.dataset.kind;
    if (delKind === 'order') deleteOrder(id);
    else if (delKind === 'product') deleteProduct(id);
    else if (delKind === 'material') deleteMaterial(id);
    else if (delKind === 'refimg') deleteRefImage(id);
    else if (delKind === 'machine' || delKind === 'ledger') deleteCostEntry(delKind, id);
    return;
  }
  ...
  if (action === 'alias-add-submit') {
    ...
    }, function(){ ui.colorAliasDraft = {alias:'', canonical:''}; });
    return;
  }
  ...
  if (action === 'sizes-save') {
    ...
    commitAndReset(function(next){ next.settings.expectedSizes = list; }, function(){ ui.expectedSizesDraft = null; });
    return;
  }
}
...
function submitNewOrder(){
  ...
  }, function(){ ui.newOrderOpen = false; ui.newOrderDraft = null; });
}

/* Aplica (o revierte) el descuento de stock de un pedido: producto vendido
   y, si tiene receta, sus materiales (ver applyMaterialsForOrderItem). Opera
   siempre sobre `target` (el `next` de un commit()), nunca sobre el estado
   real directamente. */
...
/* "Preparado" ya no se marca a mano (ver syncFulfilledFromPrep): esta
   ...
   querer o necesita reabrirlo (única llamada: data-action="undo-fulfilled"). */
function undoFulfilled(id){
  commit(function(next){
    var order = next.orders.find(function(o){ return o.id === id; });
    if (!order) return;
    order.fulfilled = false;
    order.autoFulfilled = false;
    if (order.stockApplied) applyOrderStockOnState(next, order, false, 'Reversión preparación ');
  });
}
...
function submitNewProduct(){
  ...
  commitAndReset(function(next){ next.products.push(product); },
    function(){ ui.newProductDraft = {sku:'', name:'', stock:'', threshold:'', price:''}; });
}
...
function submitEdit(id){
  ...
  }, function(){ ui.editingProductId = null; ui.editDraft = null; });
}
...
function submitAdjust(id){
  ...
  }, function(){ ui.adjustingProductId = null; ui.adjustDraft = null; });
}
...
function submitNewBlank(){
  ...
  }, function(){ ui.newBlankDraft = {color:'Blanca', size:'S', stock:'', threshold:'', unitCost:''}; });
}
...
function submitNewDtf(){
  ...
  }, function(){ ui.newDtfDraft = {name:'', detail:'', placement:'Pecho', altoCm:'', anchoCm:'', stock:'', threshold:'', imageDataUrl:''}; });
}
...
function submitMaterialAdjust(id){
  ...
  }, function(){ ui.adjustingMaterialId = null; ui.adjustMaterialDraft = null; });
}
...
function submitMaterialEdit(id){
  ...
  }, function(){ ui.editingMaterialId = null; ui.editMaterialDraft = null; });
}
...
function submitNewMachine(){
  ...
  }, function(){ ui.newMachineDraft = {date: todayISO(), name:'', amount:'', note:''}; });
}
...
function submitNewLedger(){
  ...
  }, function(){ ui.newLedgerDraft = {date: todayISO(), type:'ingreso', concept:'', category: d.category || 'Planchas (consumo)', amount:''}; });
}
...
function submitNewRefImage(){
  ...
  commitAndReset(function(next){ next.dtfImages.push(img); },
    function(){ ui.newRefImageDraft = {name:'', dataUrl:'', tags:{etiquetaCuello:false, bandera:false, escudo:false, estampado:false, blancas:false, oscuras:false}}; });
}
...
function restoreBackupConfirmed(){
  ...
  /* normalizeState() se encarga de todo el saneado y las migraciones
     (recipe de producto, prep/autoFulfilled de pedidos, altoCm/unitCost
     de materiales, tags de imágenes DTF...): así una copia de seguridad
     antigua queda tan completa como el estado normal al cargar, en vez de
     repetir aquí a mano solo una parte de esas reglas y arriesgarse a que
     se queden desincronizadas entre sí. */
  commitAndReset(function(next){
    var normalized = normalizeState(parsed);
    Object.keys(normalized).forEach(function(k){ next[k] = normalized[k]; });
  }, function(){ ui.restorePending = false; ui.restoreRaw = ''; });
}
